...
if __name__ == "__main__":
    ...
    ax.scatter(samples_xyz[:, 0], samples_xyz[:, 1], samples_xyz[:, 2], c="red", s=50)
    dec_ = float(np.mean(np.array(samples)[:,1]))
    bad_circle = np.array([radec_to_xyz(rd) for rd in [(ra, dec_) for ra in range(360)]])
    ax.scatter(bad_circle[:, 0], bad_circle[:, 1], bad_circle[:, 2], c="red", s=3)
    ...
    for dec in range(0, 90, 5):
        ...
        ax.scatter(sphere[:, 0], sphere[:, 1], sphere[:, 2], c='grey', s=3)

    for s in luddcam_catalog.stars:
        if mag := s.get('mag'):
            if mag > 3 or s['dec'] < 0:
                continue
            size = 2 * round(5 - mag * 0.6)
            xyz = radec_to_xyz((s['ra'], s['dec']))
            ax.scatter(xyz[0], xyz[1], xyz[2], c='grey', s=size)
